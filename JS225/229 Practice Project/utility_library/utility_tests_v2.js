...
function test(message, assertion) {
  ...
  try {
    ...
    passed = false;
    // throw Error(e); // Comment out to not throw errors
  }
...
// first Method - return the first element in an array
...
// without Method - returns a new array that does not contain the values passed to it
...
// range method - return an array of values starting at 0 when a single number is supplied.
// If two arguments are supplied, returns an array of values that start with the first
// argument and ends with the one number less than the second argument.
...
// Last index of - return the last index of the supplied value.
...
// Object and Object Collection Method Tests

// findWhere - return the first object with properties that match the supplied object.
// If no objects match all the supplied properties, undefined is returned.
...
// Extend - takes two or more objects, taking the properties and values from the last argument
// and adding them to the argument before it.
// Object extensions occur recursively from last argument to first.
// The first argument ends up being modified to include all properties and values from
// the other objects passed in.
