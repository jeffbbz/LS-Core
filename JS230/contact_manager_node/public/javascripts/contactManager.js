...
class ContactManager {
  ...
  requestAllContacts() {
    this.allContactsRequest = new XMLHttpRequest();
    this.allContactsRequest.open('GET', '/api/contacts');
    this.allContactsRequest.responseType = 'json';
    this.allContactsRequest.send();
  }

  InsertAllContacts() {
    this.allContactsRequest.addEventListener('load', event => {
      this.contactsRow.innerHTML = '';

      this.gatherAllTags();
      this.contacts = {contacts: this.allContactsRequest.response};
  ...
  gatherAllTags() {
    this.allContactsRequest.response.forEach(contact => {
  ...
  deleteContact(id) {
    this.requestDeleteContact(id);

    this.deleteContactRequest.addEventListener('load', event => {
      this.requestAllContacts();
      this.InsertAllContacts();
    });
  }
  ...
  editContact(id) {
    this.requestSingleContact(id);

    this.singleContactRequest.addEventListener('load', event => {
      this.contact = this.singleContactRequest.response
      ...
      this.formHeader.textContent = 'Edit Content';
      this.gatherAllTags();
      this.insertTags();
      this.showHideNonFormElements();
    });
  ...
  requestSingleContact(id) {
    this.singleContactRequest = new XMLHttpRequest();
    this.singleContactRequest.open('GET', '/api/contacts/' + id);
    this.singleContactRequest.responseType = 'json';
    this.singleContactRequest.send();
  }

  requestDeleteContact(id) {
    this.deleteContactRequest = new XMLHttpRequest();
    this.deleteContactRequest.open('DELETE', '/api/contacts/' + id);
    this.deleteContactRequest.send();
  }
  ...
  submitData(event) {
    ...
    this.submissionCleanupHandler = e => this.submissionCleanup(e, form);
    let json = this.formDataToJson(form);

    if (form.id === 'create') {
      this.postNewContact(json);
      this.postContactRequest.addEventListener('load', this.submissionCleanupHandler);
    }

    if (form.id === 'edit') {
      this.putUpdateContact(json);
      this.putUpdateContactRequest.addEventListener('load', this.submissionCleanupHandler);
    }
  ...
  putUpdateContact(data) {
    this.putUpdateContactRequest = new XMLHttpRequest();
    this.putUpdateContactRequest.open('PUT', '/api/contacts/' + this.contact.id);
    this.putUpdateContactRequest.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
    this.putUpdateContactRequest.responseType = 'json';
    this.putUpdateContactRequest.send(data);
  }
  ...
  postNewContact(data) {
    this.postContactRequest = new XMLHttpRequest();
    this.postContactRequest.open('POST', '/api/contacts');
    this.postContactRequest.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
    this.postContactRequest.responseType = 'json';
    this.postContactRequest.send(data);
  }
  ...
  search(type, param) {
    ...
    if (type === 'name') {
      if (this.searchQuery === '') {
        ...
        if (nameMatch) {
          this.hideNameNonMatches(nameMatch.getAttribute('data-id'));
          this.searchPlaceholder.style.display = 'none';
  ...
  findNameMatch(query) {
    return this.allResults.find(contact => {
