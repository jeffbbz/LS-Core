...
# Small Problems > Debugging > TF-IDF
...
The problem is very simple. In order to perform the `Math.log` method call on line 27, one of the referenced numbers needs to be a float. Currently they are both integers, the resulting integer division is off. which throws off the logarithm. However if one of them is a float, the resulting division will work correctly and the so will the logarithm. So line 24 we can chain `.to_f` to the `length` method call, converting its integer return value to a float.
...
Additionally,  if the return value of line 26 (line 3 in the above) is 0, then the Math.log call will return infinity which cannot be multiplied by anything. So can put a return of 0 if log is infinite or log if not.
