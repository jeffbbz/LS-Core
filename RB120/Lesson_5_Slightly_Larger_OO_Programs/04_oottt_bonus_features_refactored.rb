...
module Requestable
  ...
  def request_player_move(board)
    ...
    loop do
      ...
      sq = gets.chomp
      break if board.unmarked_keys.include?(sq.to_i) && sq.to_i.to_s == sq
...
class Computer < Player
  ...
  def move(board)
    choice = if player_winning?(marker, board)
               board.best_next_square(marker)
             elsif player_winning?(rival_marker, board)
  ...
  def player_winning?(player_marker, board)
