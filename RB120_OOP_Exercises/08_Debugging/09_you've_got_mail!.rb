...
As the Mailing module which includes Mail#send has not been included in the PostalService class definition or
its superclass CommuncationsProvider, PostalService class objects are unable to access that method. Additionally,
There is an Object#send method, which all objects have access to, but which require a method name as an argument. As
our first passed argument is a string not a method name, the exception is raised.

To fix this we must include the Mailing module in our class and also change the Mailing#send method to something else
so as not to accidently overwrite Object#send.
